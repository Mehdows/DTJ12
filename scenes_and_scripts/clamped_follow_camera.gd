...
@export var camera_center_marker: Marker2D 
@export var camera_radius: float = 250.0 


func _ready():
	if not camera_center_marker:
...
func _process(delta):
	...
	if not Engine.is_editor_hint() and camera_target:
		...
		if target_node:
			...
			var direction = target_pos - camera_center_marker.global_position
			if direction.length() > camera_radius:
				target_pos = camera_center_marker.global_position + direction.normalized() * camera_radius

			# Smooth camera movement
			global_position = global_position.lerp(target_pos, delta * 5.0)

# Editor visualization (Gizmo)
func _draw():
	if Engine.is_editor_hint() and camera_center_marker:
		draw_circle(camera_center_marker.global_position - global_position, camera_radius, Color(1, 0, 0, 0.5))
